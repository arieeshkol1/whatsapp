...
class _FakeTable:
    def __init__(self, items: List[Dict[str, Any]], expected_whatsapp_id: str):
        self.items = items
        self.expected_whatsapp_id = expected_whatsapp_id

    def query(self, **kwargs):
        if self.items and self.items[0].get("whatsapp_id") == self.expected_whatsapp_id:
            return {"Items": [dict(self.items[0])]}  # return a copy to avoid mutation
        return {"Items": []}
    ...
    def update_item(self, **kwargs):
        ...
        for assignment in assignments:
            ...
            self.items[0][attribute_name] = expression_attribute_values.get(placeholder)
