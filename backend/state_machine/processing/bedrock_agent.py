...
def call_bedrock_agent(*args, **kwargs) -> str:
    """Invoke the Bedrock agent and aggregate the streamed response text.

    The helper has been invoked through a few different signatures over time:

    * ``call_bedrock_agent("hi")``
    * ``call_bedrock_agent("hi", "session")``
    * ``call_bedrock_agent(input_text="hi", session_id="session")``

    To remain backward compatible we normalise the arguments here instead of
    forcing callers to adopt a specific calling convention. Only ``input_text``
    is strictly required.
    """

    session_id: Optional[str] = kwargs.pop("session_id", None)
    input_text: Optional[str] = kwargs.pop("input_text", None)

    unexpected_kwargs = ", ".join(sorted(kwargs))
    if unexpected_kwargs:
        raise TypeError(f"Unexpected keyword arguments: {unexpected_kwargs}")

    if args:
        if len(args) > 2:
            raise TypeError("call_bedrock_agent accepts up to two positional arguments")

        input_text = args[0] if input_text is None else input_text

        if len(args) > 1:
            session_id = args[1] if session_id is None else session_id
